...
type DataConverter struct {
	// Until CodecDataConverter supports workflow.ContextAware we'll store parent here.
...
// Encoder implements PayloadCodec using AES Crypt.
type Encoder struct {
	KeyID string
}
...
func (e *Encoder) Decode(payloads []*commonpb.Payload) ([]*commonpb.Payload, error) {
	result := make([]*commonpb.Payload, len(payloads))
	for i, p := range payloads {
